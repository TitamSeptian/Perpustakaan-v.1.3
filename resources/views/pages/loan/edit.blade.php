...
@section('menu') / loans / Edit  @endsection
@section('css')
...
<div class="col-md-12">
	<div class="card">
	  ...
	  <div class="card-body">
	  	<form method="POST" action="{{ route('loan.update', $pjn->id) }}">
	  		{{ csrf_field() }}
            {{ method_field('PUT') }}
	  		<div class="row">
	  			<div class="col-md-4">
	  				<div class="form-group">
						...
						<select name="id_anggota" required="" class="form-control select2">
							@foreach($anggota as $a)
								<option value="{{ $a['id'] }}" {{ ($agt->id == $a['id'] ? "selected" : '') }}>
	  			...
	  			<div class="col-md-4">
	  				<div class="form-group">
						...
						<select name="id_buku" required="" class="form-control select2">
							@foreach($buku as $b)
								<option value="{{ $b['id'] }}" {{ ($bk->id == $b['id'] ? "selected" : '') }}>
									{{ $b['judul_buku'] }}
								</option>
							@endforeach							
						</select>
	  			...
	  			<div class="col-md-4">
	  				<div class="form-group">
						...
						<input type="number" name="lama_hari" min="1" required="" class="form-control" value="{{ $pjn->lama_hari }}">
			        </div>
	  			</div>
	  		</div>
	        <div class="form-group">
